...
public class Flywheel {
    private final double targetHighSpeed = 10000;// rpm
    private final double targetLowSpeed = 5000;//rpm
    private final double highSpeedConstant = 0.8;
    private final double lowSpeedConstant = 0.4;
    private final double adjustingConstant = 1.0 / 1024.0;
    ...
    public Flywheel(TalonSRX flywheelMotor1, TalonSRX flywheelMotor2, Joystick player2) {
        ...
        this.flywheelMotor1.configSelectedFeedbackSensor(FeedbackDevice.QuadEncoder, 0, 0);
    ...
    public void run(boolean run) {
        ...
        if (((currentTime - startTime) - ((currentTime - startTime) % (dt * 1000))) > // has current time minus start time to see the relative time the trajectory has been going
            ...
            if(player2.getRawButton(1)) {
                ...
                currentPosition = (adjustingConstant * flywheelMotor1.getSelectedSensorPosition()) / (gearRatio);
            ...
            if(run) {
                ...
                flywheelMotor2.set(ControlMode.PercentOutput, speedToSetMotor);
